...
import AddSubCat from "../Forms/AddJobs/addSubCategory";
  ...
  const handleDelete = async (jobId) => {
    try {
      const response = await fetch(
        `https://efmsapi.azurewebsites.net/api/Jobs/deleteJob?jobsId=${jobId}`,
        {
          method: "GET",
          headers: {
    ...
    <div className="companiesContainer">
      <div className="companiesInner">
        <div className="companiesTop">
          <Link onClick={closeForm}>Add Sub Category</Link>
          <h1>Sub-Category</h1>
        </div>
